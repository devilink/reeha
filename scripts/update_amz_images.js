...
const PRODUCTS_DIR = path.resolve(__dirname, '../amz');
...
async function run() {
    try {
        ...
        for (const file of files) {
            ...
            // Check if matches "Label Reeha number" pattern vaguely or strict?
            // User said "Label Reeha 100.jpeg" -> "Label Reeha 100"
            // The folder is "amz", files are "Label Reeha 100.jpeg".
            // So product name is filename without ext.

            const filePath = path.join(PRODUCTS_DIR, file);
            const fileContent = fs.readFileSync(filePath);

            // Generate S3 Key
            // Use same logic as before to avoid collisions or organize? 
            // Better to keep it clean: products/{unique_id}_{filename}
            ...
            if (existingId) {
                ...
                console.log(`  Product not found in DB: ${productName}. Creating new...`);
                // Create new
